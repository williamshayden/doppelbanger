...
- Every allowed native tool invocation goes through `scripts/run_native_tool.ps1`; command spellings later in this plan are wrapper payloads, never permission to use ambient `PATH`. Native build tools use `HeadlessVst3` and import the locked MSVC/SDK environment. Validators use that profile on demand, and only the specifically requested validator is resolved. Docker commands use `StatePlaneIntegration` and never import Visual Studio state. Until `ReactEditorBuild` is designed, `node`, `npm`, and `npx` reject with `DBDOC_TOOL_PROFILE_REQUIRED`.
...
`-ExecutionPolicy Bypass` is process-scoped. The doctor reads common registry, disk, Git, ownership, process-ancestry, and PE evidence plus only the selected profile's tool metadata. `HeadlessVst3` inventories native build tools; `StatePlaneIntegration` inventories WSL, Docker, Compose config, and plugin provenance. Only `Compatibility` inventories Node, WebView2, and Ableton. It does not install, start services, change PATH/configuration, or write unless `-ReportPath` names an ignored path.

`scripts/run_native_tool.ps1` accepts a tool name followed by its arguments and selects exactly one profile. Native build tools select `HeadlessVst3`, prepend the absolute CMake/Ninja/Rust paths from the lock, locate the exact VS 17.14 installation with `vswhere.exe`, import `VsDevCmd.bat -arch=x64 -host_arch=x64 -vcvars_ver=14.44 -winsdk=10.0.26100.0`, and resolve the requested executable again. Validators are on-demand: only the specifically requested validator is resolved under its locked root or canonical repo build directory after provenance checks. Docker selects `StatePlaneIntegration` and never imports `VsDevCmd.bat` or resolves compiler tools; it resolves Docker Desktop's absolute `docker.exe`, verifies CLI 29.6.2 and PE provenance, rechecks Compose provenance on every invocation, and permits only the Linux/amd64 `desktop-linux` server. Until a future profile exists, `node`, `npm`, and `npx` reject with `DBDOC_TOOL_PROFILE_REQUIRED`. Every allowed executable is checked as PE32/PE32+ before launch; WSL/GNU executables are forbidden. `-Describe` emits the selected profile, paths, and environment without executing a tool.
...
- [ ] Add the toolchain files and implement pure probe-validation functions `Read-ToolchainLock`, `Get-NativeWindowsProbe`, `Test-NativeWindowsProbe`, and `Write-DoctorReport`. Every profile rejects an active WSL environment or `wsl.exe`, `wslhost.exe`, or `bash.exe` ancestor and requires a drive-letter NTFS repo rather than UNC/`\\wsl$`. `HeadlessVst3` requires `rustc -vV` to report the MSVC host, `cl /Bv` to report 19.44, and every resolved native build binary to be PE32/PE32+ AMD64, while succeeding independently of installed WSL and Docker state. `StatePlaneIntegration` permits native `docker.exe` to address Docker Desktop's Linux WSL2 engine without resolving native compiler tools. Only `Compatibility` inventories Node, WebView2, and Ableton.
...
- [ ] Run each real doctor profile separately. `HeadlessVst3` initially reports missing Rust/MSVC/SDK/CMake/Ninja but no Node or Docker diagnostic, and `HeadlessVst3` succeeds independently of Docker state once native tools are exact. `StatePlaneIntegration` owns stopped/drifted Docker and Compose-shadow diagnostics. `Compatibility` alone inventories Node, WebView2, and Ableton. No profile attempts WSL compilation.
...
- [ ] Run the real `doctor_windows.ps1 -Profile HeadlessVst3 -Json -ReportPath var/validation/m0-windows/doctor.json`. Expected: all compile tools pass independently of Docker state, with no Docker diagnostic.
- [ ] Run `doctor_windows.ps1 -Profile StatePlaneIntegration -Json` while Docker Desktop is stopped and require `DBDOC_DOCKER_STOPPED`. Then start Docker Desktop normally, wait for the `desktop-linux` context, and rerun `StatePlaneIntegration`. Require the locked Engine/CLI 29.6.2 and Compose 5.3.1 inherited by Docker Desktop 4.85.0; stop on any mismatch instead of rewriting the lock to match the machine.
...
- [ ] Run API integration in an isolated Compose project, always tearing down only that project. The integration harness explicitly composes `HeadlessVst3` for the native Cargo test and `StatePlaneIntegration` for the Compose lifecycle; it must never infer Docker requirements from Cargo arguments:
...
Record decision `PD-033` with these exact choices:
...
- [ ] Add failing doc assertions for `PD-033`, 10 ms, output safety behavior, out-of-place alias rules, IDs, `kSample32`, and full-block fault semantics.
...
- [ ] Create a reusable recursive dependency-closure gate and apply it to every native executable and DLL in the UI-NONE VST3 bundle. Parse normal imports and delay-load imports, require AMD64 PE provenance, reject toolchain runtime DLLs that should have been statically linked, and permit each dependency only when it is in a committed reviewed Windows system-DLL allowlist or contained within the bundle. Reject bundle escape through DLL search. Derive the allowlist from documented Windows platform requirements and reviewed code usage; explicitly forbid auto-populating it from the first artifact's observed imports. The current bundle is expected to contain only the plugin module, and the later native companion must reuse this gate.
...
- [ ] Include the same recursive dependency-closure inventory in validator evidence. Reject WebView, Node, Docker, WSL, database, service, or compiler leakage from UI-NONE artifacts.
- [ ] Run:
...
- [ ] Install and run the bundle on a disposable, network-disconnected clean Windows 11 x64 VM with no WSL, Docker, Rust, Visual Studio, CMake, Ninja, Node, Postgres, or PostgREST. Use a statically linked pinned Steinberg headless host with its own reviewed dependency closure, safe DLL search, a separate host directory, and a one-process Windows job. Before loading, record the OS image, disabled WSL and Virtual Machine Platform features, absence of WSL distributions and container, database, and developer files, services, processes, PATH entries, and installed programs, plus host and bundle hashes and disconnected-network state. Begin continuous image-load and process tracing from before host launch through termination, and preserve explicit loaded-module snapshots before load and after initialization, processing, restore, and unload. Exercise load, stereo processing, settled bypass, state restore, and unload; record every runtime-loaded module's canonical path and hash at each phase, plus the process tree, listeners, and filesystem changes. The gate must statically reject dynamic-loader imports: `LoadLibrary*` and `GetProcAddress` are forbidden unless each use is declared in a committed manifest and covered by call-level instrumentation that records the requested DLL and symbol and the resolved canonical module path. Reject any undeclared or uninstrumented dynamic-loader use, unresolved or undeclared module, writable, PATH, or network search location, child process, download, prerequisite installation, listener, or unexpected filesystem write.
...
- [ ] Rerun the UI-NONE VST3 normal-import, delay-load, runtime-loaded-module, and clean-target gates as final release evidence. Verify that their contract is reusable by the later native-companion milestone, but do not require or execute companion packaging during the UI-NONE milestone.
