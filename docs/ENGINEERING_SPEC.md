...
| iPlug2 wrapper | VST3 lifecycle, parameters, editor bridge, DAW state, fixed capture handoff, background control client | mastering math, React rendering |
| React editor | local presentation, parameter gestures, capture/service command initiation, accessible error and progress states | audio buffers, direct PostgREST or filesystem access, mastering math, durable host state |
...
The first host proof has no custom editor and uses Ableton's generic parameter surface. The first product editor is React and TypeScript compiled to local static assets and hosted by iPlug2's platform WebView. A versioned C++ bridge owns parameter gestures, snapshots, file-dialog requests, and controller/background work. React, JSON parsing, WebView calls, and JavaScript execution are forbidden from the audio callback. The editor loads no remote runtime content.
...
- A zero-length host call may flush at most one offset-zero point for each stable parameter with null audio buffers; it updates parameter targets without advancing DSP state, and smoothing begins on the next nonzero block.
...
- Construction, plan validation, file I/O, analysis, and preparation of fixed coefficient/amplitude lookup tables happen outside the callback. Fixed-topology automation selects precomputed centidecibel targets and advances Rust-owned smoothing without coefficient or transcendental calculation in the callback.
...
Every job declares one request kind and follows exactly one state machine:

```text
plan_only:          queued -> analyzing -> plan_ready
                    \------------------------> failed

validation_render: queued -> analyzing -> plan_ready
                    plan_ready -- explicit render claim --> rendering -> complete
                    \----------------------------------------------------> failed
```

`plan_only` requires no render destination and never creates an output WAV. `validation_render` writes only beneath the managed output root and persists a report tied to the exact plan hash. Its worker must commit `plan_ready` and stop; only an explicit idempotent claim carrying that hash may advance it to `rendering`, so polling cannot miss the plan state and a crashed caller can resume. In the plugin workflow, analysis first uses `plan_only`; a later report request may use `validation_render` with the current validated plan and completed target capture. That render is measurement evidence, not the user's DAW export, and it may not replace the plugin's active plan.
...
- A separate browser workshop UI is not part of the product plan; the bundled React editor is the VST3's local product interface.
